...
def random_points_on_unit_hypersphere(k, dim):
    angles = np.random.uniform(0, 2 * np.pi, size=(k, dim - 1))
    coordinates = np.column_stack([np.cos(angles[:, i]) for i in range(dim - 1)])
    coordinates = np.column_stack([coordinates, np.sin(angles[:, -1])])
    return coordinates
...
if __name__ == '__main__':
    ...
    parser.add_argument('n_cluster', help='n_cluster')
    ...
    random_points = random_points_on_unit_hypersphere(n_cluster, n_dim)
    min_distance = calculate_min_distance(random_points)

    for i in range(99999):
        random_points_i = random_points_on_unit_hypersphere(n_cluster, n_dim)
        min_distance_i = calculate_min_distance(random_points_i)

        if min_distance_i > min_distance:
            min_distance = min_distance_i
            random_points = random_points_i

    # 手動で設定する共分散行列
    covariances = [np.identity(n_dim) * calculate_min_distance(random_points) / (50 * 2) for _ in range(n_cluster)]

    # ガウス混合分布を手動で作成
    gaussian_mixtures = [multivariate_normal(mean=random_points[i], cov=covariances[i]) for i in range(n_cluster)]

    # データを生成
    data_points = np.concatenate([gaussian_mixtures[i].rvs(size= n_nodes // n_cluster) for i in range(n_cluster)]) * 10
    ...
    adj_mat = distance_mat(data_points, data_points)

    # probability matrix
    adj_mat = connection_prob(adj_mat, R=params_dataset['R'], beta=params_dataset['beta'])
    ...
    for model_k in [1,2,4,8]:
        ...
        filepath = "result_artificial_gmm_1.csv"
